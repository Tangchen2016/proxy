...
source $current_dir/get_trojan_config.sh
filePath='/var/tmp'
fileName='$filePath/vmess'
clashXfileName='$filePath/clashX.yaml'
...
init() {
  ...
  $current_dir/busybox httpd -p 0.0.0.0:$port -h $filePath

  random

  echo "* * * * * $current_dir/proxy.sh random " >/var/spool/cron/root
  crontab -u root /var/spool/cron/root

  echo "[+]定时任务添加成功，每分钟更新一次trojan端口"

  echo "[+]你的trojan伪动态vmess订阅链接为：http://$publicIp:$port/vmess"
  echo "[+]你的trojan伪动态clashX订阅链接为：http://$publicIp:$port/clashX.yaml"
}

random() {
  # 随机生成10000~20000之间的一个随机数
  randPort=$((RANDOM % 10000 + 10000))
  local config_data=($(read_config_file))
  local num_lines=$((${#config_data[@]} / 4))
  local trojan_links=""
  local clashX_links=""
  local node_links=""
  for ((i = 0; i < num_lines; i++)); do
    local start_index=$((i * 4))
    local node_name=${config_data[start_index]}
    local host=${config_data[start_index + 1]}
    local port=${config_data[start_index + 2]}
    local password=${config_data[start_index + 3]}
    echo "node_name: $node_name"
    echo "host: $host"
    echo "port: $port"
    echo "password: $password"
    trojan_links+="trojan://$password@$host:$randPort#$node_name\n"
    clashX_links+="- name: $node_name
  type: trojan
  server: $host
  port: $randPort
  password: $password
...
"
  ...
  # 把你的trojan链接放到下面
  echo -e "$trojan_links" | base64 >$fileName
...
chmod +x trojan.sh setup.sh dynamic_port.sh proxy.sh